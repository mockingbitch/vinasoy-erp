...
<div class="card mb-4">
    ...
    <div class="form-group mt-4">
        ...
        <input type="text" name="tenSP" class="form-control" id="inputTenSP" aria-describedby="nameHelp" value="{{Request::old('tenSP')}}">
    </div>
    <div class="row">
        <div class="form-group mt-4 col-6">
            ...
            <select name="danhmuc_id" style="width:20%;height:50px;margin-left:17px " class="select form-select-lg mb-3" aria-label=".form-select-lg example">
                @foreach ($listDanhMuc as $danhMuc)
                    <option value="{{$danhMuc->id}}" {{Request::old('danhmuc_id') == $danhMuc->id ? 'selected' : ''}}>{{$danhMuc->tenDanhMuc}}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group mt-4 col-6">
            ...
            <select name="nhacungcap_id" style="width:20%;height:50px;margin-left:17px " class="select form-select-lg mb-3" aria-label=".form-select-lg example">
                @foreach ($listNhaCC as $nhacc)
                    <option value="{{$nhacc->id}}" {{Request::old('nhacungcap_id') == $nhacc->id ? 'selected' : ''}}>{{$nhacc->tenNhaCC}}</option>
                @endforeach
            </select>
        </div>
    </div>
    <div class="form-group mt-4">
        ...
        <input type="number" name="donGia" class="form-control" id="inputDonGia" aria-describedby="nameHelp" value="{{Request::old('donGia')}}">
    </div>
    <div class="form-group mt-4">
        <label for="inputHinhAnh">Hình ảnh @if ($errors->has('img'))<p class="text-error">*{{$errors->first('img')}}</p>@endif</label>
        <input type="file" name="img" class="form-control" id="inputHinhAnh" aria-describedby="nameHelp" value="{{Request::old('img')}}">
    </div>
    <div class="form-group mt-4">
